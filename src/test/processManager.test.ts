...
suite("ProcessManager Tests", () => {
  ...
  suite("executeAdbCommand", () => {
    test("should execute ADB command successfully", async () => {
      ...
      setTimeout(() => {
        ...
        mockProcess.stdout.emit(
          "data",
          Buffer.from("192.168.1.100:5555\tdevice\n")
        );
      ...
      assert.ok(
        mockLogger.info.calledWith(
          "Executing ADB command: /path/to/adb devices"
        )
      );
      assert.ok(
        mockLogger.info.calledWith("ADB command completed with exit code: 0")
      );
    });
    ...
    test("should handle ADB command failure", async () => {
      ...
      const commandPromise = processManager.executeAdbCommand([
        "connect",
        "192.168.1.100:5555",
      ]);
      ...
      setTimeout(() => {
        mockProcess.stderr.emit(
          "data",
          Buffer.from("failed to connect to 192.168.1.100:5555\n")
        );
    ...
    test("should log process output", async () => {
      ...
      assert.ok(
        mockLogger.logProcessOutput.calledWith("adb", "device output\n")
      );
      assert.ok(
        mockLogger.logProcessOutput.calledWith("adb", "error output\n")
      );
    });
  });
  ...
  suite("launchScrcpy", () => {
    test("should launch scrcpy successfully", async () => {
      ...
      // Verify logging
      assert.ok(
        mockLogger.info.calledWith("Launching scrcpy: /path/to/scrcpy ")
      );
    });
    ...
    test("should timeout if scrcpy fails to start", async () => {
      ...
      try {
        ...
        assert.strictEqual(
          error.message,
          "Scrcpy failed to start within timeout period"
        );
      }
  ...
  suite("cleanup", () => {
    test("should clean up all managed processes", async () => {
      ...
      // Verify logging
      assert.ok(
        mockLogger.info.calledWith("Cleaning up all managed processes")
      );
  ...
  suite("buildScrcpyArgs", () => {
    ...
    test("should build args with partial options", async () => {
      ...
      const expectedArgs = ["--bit-rate", "2000000", "--max-size", "720"];
      assert.ok(spawnStub.calledWith("/path/to/scrcpy", expectedArgs));
    });
  });
});
